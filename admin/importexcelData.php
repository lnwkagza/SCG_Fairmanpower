    ...
    if (in_array($file_ext, $allowed_ext)) {
        ...
        foreach ($data as $row) {
            ...
            if ($count > 1) {
                ...
                $work_format_code = $row[45]; // AT

                $prevQuery = "SELECT * FROM employee WHERE card_id = ?";
                $params = array($card_id);
                $prevResult = sqlsrv_query($conn, $prevQuery, $params);


                if (sqlsrv_has_rows($prevResult)) {

                    // ถ้า query แล้วมันมี card_id อยู่แล้วเราจะแจ้งกลับไปให้ ajax เผื่อถามคำยืนยันว่าจะอัปเดตหรือไม่
                    echo json_encode(array(
                        'status' => 'update_confirm',
                        'card_id' => $card_id,
                        'message' => 'พบข้อมูลพนักงานที่มีรหัสบัตร ' . $card_id . ' อยู่แล้ว ต้องการอัปเดตข้อมูลหรือไม่?'
                    ));
                    exit();
                } else {

                    // ถ้า query แล้วมันไม่มี card_id อยู่แล้ว แถวนั้นจะถูก INSERT เข้าตาราง Employee
                    $insertQuery = "INSERT INTO employee 
                        ...
                        permission_id, work_format_code) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    $insertParams = array(
                        $card_id, $person_id, $personnel_number, $scg_employee_id,
                        $prefix_thai, $firstname_thai, $lastname_thai,
