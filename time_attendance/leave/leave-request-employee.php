...
<!-- CSS Desktop -->
<link rel="stylesheet" href="../components-desktop/employee/leave-request-employee.css">
...
if (sqlsrv_has_rows($sql_absence)) {
    ...
    // Assuming $row contains the 'gender' column value
    $gender = $row["gender"];

    if ($gender == "ชาย") {
        $sql_emp = sqlsrv_query($conn, "SELECT * FROM absence_type WHERE absence_type_id NOT IN (4, 14)");
    } elseif ($gender == "หญิง") {
        $sql_emp = sqlsrv_query($conn, "SELECT * FROM absence_type WHERE absence_type_id NOT IN (9, 12, 13, 15)");
    }
...
function add_leave_confirm() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            form.action = '../processing/process_Leave_Request.php';
            form.method = 'POST';
            form.submit();
        }
    });
...
function add_leave_cancel_confirm() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = "leave-rights-employee.php";
        }
    });
}
...
<body>
    ...
    <div style="display:none;" id="Content" class="animate-bottom">
        ...
        <div class="mobile">
            ...
            <form id="leaveForm" action="../processing/process_Leave_Request.php" method="post"
