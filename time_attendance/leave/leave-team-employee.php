...
<!-- CSS Desktop -->
<link rel="stylesheet" href="../components-desktop/employee/leave-team-employee.css">
...
if (!isset ($department)) {
    ...
    $query = "SELECT 
            ...
            FROM manager
            LEFT JOIN employee ON manager.card_id = employee.card_id
            LEFT JOIN cost_center ON employee.cost_center_organization_id = cost_center.cost_center_id
            LEFT JOIN section ON cost_center.section_id = section.section_id 
            LEFT JOIN department ON section.department_id = department.department_id
            LEFT JOIN division ON department.division_id = division.division_id
            LEFT JOIN work_format ON employee.work_format_code = work_format.work_format_code
            WHERE manager.manager_card_id = ? and employee.sub_team_id in (SELECT sub_team_id FROM employee WHERE card_id = ?) ";
    $params = array($card_id_row["manager_card_id"], $_SESSION['card_id']);
    $stmt = sqlsrv_query($conn, $query, $params);
} else {
    $query = "SELECT 
                manager.manager_card_id,
            ...
            LEFT JOIN division ON department.division_id = division.division_id
            LEFT JOIN work_format ON employee.work_format_code = work_format.work_format_code
            WHERE manager.manager_card_id = ? and employee.cost_center_organization_id = ? and employee.sub_team_id in (SELECT sub_team_id FROM employee WHERE card_id = ?)";
    $params = array($card_id_row["manager_card_id"], $department, $_SESSION['card_id']);
    $stmt = sqlsrv_query($conn, $query, $params);
}
                                            ...
                                            <div class="form">
                                                ...
                                                <div class="form-date">
                                                    <input class="form-control" type="date" id="startDate"
                                                        name="startDate" value="<?php echo $start_mon; ?>">

                                                    <label for="endDate">ถึง</label>
                                                    <input class="form-control" type="date" id="endDate" name="endDate"
                                                        value="<?php echo $end_mon; ?>">
                                                </div>
                                                            ...
                                                            <td class="employee">
                                                                <div class="row">
                                                                    <div style="margin-right: 10px;margin-left: 5px;">
                                                                        <img src="<?php echo (!empty ($employee['employee_image'])) ? '../../admin/uploads_img/' . $employee['employee_image'] : '../IMG/user.png'; ?>"
                                                                            class="border-radius-100 shadow" width="50px"
                                                                            height="50px" alt="">
                                                                    </div>
                                                                    <div>
                                                                        <b>
                                                                        ...
                                                                        <b class="department"
                                                                            data-short="<?= "แผนก: " . htmlentities(mb_strimwidth($employee['department'], 0, 23, '...')) ?>"
                                                                            data-full="<?= "แผนก: " . htmlentities($employee['department']) ?>">
