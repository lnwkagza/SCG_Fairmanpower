...
if (!empty($_SESSION["card_id"])) {
    ...
    $sql_day_off_request = sqlsrv_query($conn, "SELECT 
        ...
        INNER JOIN employee AS emp1 ON emp1.card_id = day_off_request.card_id
        INNER JOIN cost_center AS emp1_cost_center ON emp1_cost_center.cost_center_id = emp1.cost_center_payment_id
        INNER JOIN section AS emp1_section ON emp1_section.section_id = emp1_cost_center.section_id
        INNER JOIN position_info AS emp1_position_info ON emp1_position_info.card_id = emp1.card_id
        INNER JOIN position AS emp1_position ON emp1_position.position_id = emp1_position_info.position_id
        INNER JOIN employee AS emp2 ON emp2.card_id = day_off_request.request_card_id
        INNER JOIN cost_center AS emp2_cost_center ON emp2_cost_center.cost_center_id = emp2.cost_center_payment_id
        INNER JOIN section AS emp2_section ON emp2_section.section_id = emp2_cost_center.section_id
        INNER JOIN position_info AS emp2_position_info ON emp2_position_info.card_id = emp2.card_id
        INNER JOIN position AS emp2_position ON emp2_position.position_id = emp2_position_info.position_id
