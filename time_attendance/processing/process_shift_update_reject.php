...
if (!empty($shift_change_id)) {
    ...
    $addvalues = array('reject', $_SESSION["card_id"], $time_stamp, $shift_change_id);
...
if (!empty($shift_add_id)) {
    $query = "UPDATE shift_add 
              SET approve_status = ?, approver = ?, approve_time = ? 
              WHERE shift_add_id = ?";
    $addvalues = array('reject', $_SESSION["card_id"], $time_stamp, $shift_add_id);
    $stmt = sqlsrv_prepare($conn, $query, $addvalues);
...
if (!empty($shift_lock_id)) {
    $query = "UPDATE shift_lock 
              SET approve_status = ?, approver = ?, approve_time = ? 
              WHERE shift_lock_id = ?";
    $addvalues = array('reject', $_SESSION["card_id"], $time_stamp, $shift_lock_id);
    $stmt = sqlsrv_prepare($conn, $query, $addvalues);
...
if (!empty($shift_switch_id)) {
    ...
    $addvalues = array('reject', $_SESSION["card_id"], $time_stamp, $shift_switch_id);
    $stmt = sqlsrv_prepare($conn, $query, $addvalues);
...
if (!empty($sub_team_id)) {
    $query = "UPDATE sub_team 
              SET approve_status = ?, approver = ?, approve_time = ? 
              WHERE sub_team_id = ?";
    $addvalues = array('reject', $_SESSION["card_id"], $time_stamp, $sub_team_id);
    $stmt = sqlsrv_prepare($conn, $query, $addvalues);
