...
jQuery(() => {
    ...
    // $('#select-Time-container').hide();
    $('#select-Coords-container').hide();
    $('#summary-data-btn').hide();
...
function updateEmployeeList() {
    ...
    $.each(employeesList, function (index, employee) {
        let listItem = $('<li class="employeeUL"><button type="button" class="btn btn-danger remove-employee input-box" data-index="' + index + '">ลบ</button> ' + employee.scg_id + ' (' + employee.name + ') </li>');
...
$('#next-step-btn').on("click", function () {
    ...
    $('#select-Employees-container').hide();
    $('#select-Time-container').hide();

    $('#next-step-btn').hide();

    $('#select-Coords-container').show();
    $('#summary-data-btn').show();
...
function ShowConfirmBTN() {
    console.log('ยืนยันหรือไม่');
    $('#submit-form-button').show();
    $('#cancel-form-button').show();
    $('#confirm-form-button').hide();

}

function CancelConfirmBTN() {
    console.warn('ยกเลิกแล้ว');
    $('#submit-form-button').hide();
    $('#cancel-form-button').hide();
    $('#confirm-form-button').show();
}
...
//         console.log('Datepicker input field is not empty.');

//     } else {

//         console.log('Datepicker input field is empty.');
...
$('#submit-form-button').on('click', function () {
    ...
    if (TIME_checkedRadio.attr("id") === "shift_time1") {
        ...
        if ($('#shift_start_date').val().length <= 0) {
        ...
        if ($('#shift_start_date').val().length <= 0 && $('#shift_end_date').val().length <= 0) {
        ...
        Swal.fire({
            title: "เกิดข้อผิดพลาด",
            text: "คุณยังไม่ได้เลือกะยะเวลาทำงาน",
            icon: "error"
        });
        return;
    }
    ...
    if (GPS_checkedRadio.attr("id") === "gps_type2" && (check_in_coordinates.coordinates && check_out_coordinates.coordinates)) {
        ...
        Swal.fire({
            title: "เกิดข้อผิดพลาด",
            text: "คุณยังไม่ได้เลือกประเภทการลงชื่อ",
            icon: "error"
        });
        return;
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response === "work-shift-tempo-success") {
                Swal.fire({
                    title: "เสร็จสิ้น",
                    text: "คุณได้ส่งกำหนดพิกัดทำงานนอกโรงงานแล้ว.",
                    icon: "success",
                    timer: 2000,
                }).then((result) => {
                    if (result.dismiss === Swal.DismissReason.timer) {
