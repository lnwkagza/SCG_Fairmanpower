...
require_once('inspector_gps_approval_query.php');
            ...
            <div id="div-inspector">
                ...
                <div class="display-transection">
                    ...
                    <div class="topic-manage">
                        <span class="cost-center">จัดการคำร้องทำงานนอกสถานที่[ผู้ตรวจสอบ]</span>
                    </div>

                    <div class="mobile-table">

                        <table id="employee_request_gps_inspector" class="displayTB table table-striped table-bordered">
                            <thead class="table-dark">
                                <tr>
                                    <th hidden>รหัสคำร้อง</th>
                            ...
                            <tbody>
                                <?php while ($row = sqlsrv_fetch_array($inspector_result, SQLSRV_FETCH_ASSOC)) {

                                    $sql_approval = "SELECT * FROM approval_status
                                                        WHERE approval_id != ?
                                                        AND approval_id NOT IN (1,3,4)
                                                    ";
                                ...
                                ?>
                                    <tr>
                                        <td hidden class="tb_shift_id" value="<?= $row['shift_id']; ?>"><?= $row['shift_id']; ?>
                                        </td>

                                        <td hidden class="tb_employee_id"><?= $row['request_card_id']; ?></td>

                                        <td class="tb_coords_name"><?= $row['coords_name']; ?></td>

                                        <td class="tb_employee_name"><?= $employee_name ?></td>
                                        ...
                                        <td hidden class="tb_start_date"><?= $row['shift_start_date']->format('Y-m-d') ?></td>
                                        ...
                                        <td>
                                            <select class="approval_status_selection">
        ...
        if (closestSelected === '2') {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '../gps/gps_update_approval_inspector.php',
                        method: 'POST',
                        data: {
                            shift_id: shift_id,
                            card_id: cardId,
